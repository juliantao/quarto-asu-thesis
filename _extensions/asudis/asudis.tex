...
$if(thesis.titlelines)$
$for(thesis.titlelines)$
\listadd{\titlelines}{$thesis.titlelines$}             
$endfor$
$else$
\listadd{\titlelines}{$title$}             
$endif$

\newcommand*\Author{$author$}          %<Enter your name; must match official transcript
\newcommand*{\documentname}%
  {$thesis.doctype$}                        %<Enter the type of document (capitalized)
\newcommand*{\degreename}
  {$thesis.degreetype$}                %<Enter the type of degree (capitalized)
\newcommand*\defdate{$thesis.defdate$}        %<Give month (written out fully) and year of
                                        %   the oral defense
$for(thesis.committeechair)$
\listadd{\committeechair}{$thesis.committeechair$}   %<Enter committee chair name; use \listadd for
$endfor$
...
\newcommand*{\chairlabel}{Chair(s)}        %<If you have co-chairs, replace this text with
                                        %   'Co-Chair'
$for(thesis.committeemember)$
\listadd{\committeemember}{$thesis.committeemember$} %<Enter committee member names; use \listadd for
$endfor$
\newcommand*{\gradmonth}{$thesis.gradmonth$}           %<Enter the graduation date; month can only be:
                                        %  May, August, or December
\newcommand*{\gradyear}{$thesis.gradyear$}           %<Enter the graduation year, e.g. 2014

$for(thesis.keywords)$
\listadd{\keywords}{$thesis.keywords$}          %<Enter keywords; use \listadd for
$endfor$
...
$if(thesis.frontmatter.dedication)$
\chapter*{Dedication}
\input{$thesis.frontmatter.dedication$}
$endif$

$if(thesis.frontmatter.acknowledgments)$
\chapter*{Acknowledgments}
\input{$thesis.frontmatter.acknowledgments$}
$endif$
...
\input{$thesis.frontmatter.definitions$}                    %<Enter the name of the .tex file or omit this
%   line and type in here.
$endif$

$if(thesis.frontmatter.preface)$
\chapter{Preface}                      %~PREFACE (optional, less than 10 pages)
\input{$thesis.frontmatter.preface$}                        %<Enter the name of the .tex file or omit this
$endif$
...
$if(thesis.backmatter.biosketch)$
\biographicalsketch{\input{$thesis.backmatter.biosketch$}}
$endif$
